...
{
  ros::init(argc, argv, "clusteringLeft");
  ros::NodeHandle nh;

  // << Subscribe Topic >>
  // topic name : /filterLeft
  // topic type : sensor_msgs::PointCloud2
  ros::Subscriber sub = nh.subscribe<sensor_msgs::PointCloud2>("/filterLeft", 1, callbackFcn);

  // << Publish Topic >>
  // topic name : /clusterLeft
  // topic type : sensor_msgs::PointCloud2
  pub = nh.advertise<sensor_msgs::PointCloud2>("/clusterLeft", 1);
